...
def overlay_vis_graph(Thymio_center, Thymio_target, obs_corners, optimal_path, warpedimg_clean):
    ...
    for i in range(len(optimal_path)-1):
        cv2.line(warpedimg_clean_cpy,tuple([optimal_path[i][1][1],optimal_path[i][1][0]]),
                tuple([optimal_path[i+1][1][1],optimal_path[i+1][1][0]]),
                color=(119, 172, 48),
                thickness=12)
    cv2.putText(warpedimg_clean_cpy, 'Start', tuple([Thymio_center[1],Thymio_center[0]]), fontFace = cv2.FONT_HERSHEY_SIMPLEX, fontScale=1.5, color = (0,0,0),thickness = 7)
